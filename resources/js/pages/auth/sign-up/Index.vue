  ...
  setup() {
    ...
    const handleStep = handleSubmit((values) => {
      ...
      if (currentStepIndex.value === 0) {
        ...
        // Langkah 2: Verifikasi OTP Email, lalu langsung lanjut ke langkah password
        checkOtpEmail(() => {
          formData.value = { ...values };

          currentStepIndex.value++; // Langsung ke langkah password

          if (!_stepperObj.value) {
            return;
          }

          _stepperObj.value.goNext(); // Pindah ke langkah password
        });
      } else if (currentStepIndex.value === 2) {
        // Langkah 3: Proses password
        formData.value = { ...values };
